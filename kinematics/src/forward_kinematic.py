#!/usr/bin/env python3
"""
Forward Kinematics Module for 6-DOF Robot Manipulator with Tool Support
...
pose given joint angles, with optional tool transformation support.

Key Features:
- Product of Exponentials (PoE) formulation
- Screw theory implementation
- Matrix exponential computation
- Tool/gripper transformation support
...
# Import gripper tool support
try:
    import sys
    sys.path.append(os.path.join(os.path.dirname(__file__), '..', '..'))
    from tool.gripper import GripperTool
    GRIPPER_SUPPORT = True
except ImportError:
    GRIPPER_SUPPORT = False
    GripperTool = None
...
class ForwardKinematics:
    ...
    def __init__(self, constraints_path: Optional[str] = None, tool_name: Optional[str] = None):
        """
        ...
        Args:
            ...
            tool_name: Name of tool/gripper to attach (None for TCP-only mode)
        """
        ...
        if tool_name and GRIPPER_SUPPORT:
            try:
                self.tool = GripperTool(self.constraints_path, tool_name)
                self.tool_mode = True
                logger.info(f"Tool '{tool_name}' attached successfully")
            except Exception as e:
                logger.warning(f"Failed to attach tool '{tool_name}': {e}")
                logger.info("Continuing in TCP-only mode")
        elif tool_name and not GRIPPER_SUPPORT:
            logger.warning("Tool requested but gripper support not available")
        ...
        mode_str = "with tool" if self.tool_mode else "TCP-only"
        logger.info(f"Constraint-free forward kinematics initialized with {self.n_joints} joints ({mode_str})")
    ...
    def compute_forward_kinematics(self, q: np.ndarray) -> np.ndarray:
        ...
        # Apply tool transformation if tool is attached
        if self.tool_mode and self.tool:
            T = self.tool.transform_tcp_to_tool(T)
        
        return T
    
    def compute_tcp_kinematics(self, q: np.ndarray) -> np.ndarray:
        """
        Compute forward kinematics to TCP (without tool transformation).
        
        This method always returns the TCP pose regardless of tool attachment.
        Useful for debugging or when you specifically need TCP coordinates.
        
        Args:
            q: Joint angles in radians (n_joints,)
            
        Returns:
            4x4 homogeneous transformation matrix of TCP pose
        """
        if not isinstance(q, np.ndarray) or q.ndim != 1 or q.shape[0] != self.n_joints:
            raise ForwardKinematicsError(f"Invalid joint configuration. Expected ({self.n_joints},), got {q.shape}")
    ...
    def attach_tool(self, tool_name: str) -> bool:
        """
        Attach a tool to the robot.
        
        Args:
            tool_name: Name of tool to attach
            
        Returns:
            True if tool attached successfully, False otherwise
        """
        if not GRIPPER_SUPPORT:
            logger.warning("Tool attachment requested but gripper support not available")
            return False
    ...
    def detach_tool(self):
        """Detach the current tool (switch to TCP-only mode)."""
        if self.tool_mode:
            tool_name = self.tool.tool_name if self.tool else "unknown"
            self.tool = None
            self.tool_mode = False
            logger.info(f"Tool '{tool_name}' detached - switched to TCP-only mode")
        else:
            logger.info("No tool attached - already in TCP-only mode")
    
    def is_tool_attached(self) -> bool:
        """Check if a tool is currently attached."""
        return self.tool_mode and self.tool is not None
    
    def get_tool_info(self) -> Optional[dict]:
        """Get information about the currently attached tool."""
        if self.tool_mode and self.tool:
            return self.tool.get_tool_info()
        return None
    
    def get_tool_offset(self) -> Optional[np.ndarray]:
        """Get the tool offset transformation matrix."""
        if self.tool_mode and self.tool:
            return self.tool.tcp_to_tool_transform.copy()
        return None