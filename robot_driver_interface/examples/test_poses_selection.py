...
This script demonstrates the visualizer_tool_TCP tool for interactive
...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..', 'planning', 'examples'))
...
def run_pick_place_pose_demo():
    ...
    try:
        # Import motion planning components with better error handling
        try:
        ...
        visualizer = visualizer_tool_TCP()
        visualizer.run()
        ...
        if plan.success:
            ...
            # FK Validation setup
            if ForwardKinematics is not None:
            ...
            for i, waypoint in enumerate(plan.waypoints):
                ...
                print(f"         Expected TCP: [{waypoint.tcp_position_mm[0]:.1f}, {waypoint.tcp_position_mm[1]:.1f}, {waypoint.tcp_position_mm[2]:.1f}] mm")
